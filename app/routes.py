...
@routes.route('/activity_analytics')
@login_required
def activity_analytics():
    analytics = current_user.get_activity_analytics()
    ...
    analytics['monthly_frequency'] = recent_sessions
    analytics['weekly_average'] = round((recent_sessions * 7) / 30, 1)
    
    return render_template('activity_analytics.html', 
                         analytics=analytics,
                         user=current_user)

@routes.route('/workout_streaks')
@login_required
def workout_streaks():
    streak_data = current_user.get_workout_streaks()
    return render_template('workout_streaks.html', 
